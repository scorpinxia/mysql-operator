...
func (s *mergeFromPatch) Data(obj runtime.Object) ([]byte, error) {
	...
	if s.opts.OptimisticLock {
		...
		version, _ := accessor.ResourceVersion(original)
		if len(version) == 0 {
			return nil, fmt.Errorf("cannot use OptimisticLock, object %q does not have any resource version we can use", original)
		}

		original = original.DeepCopyObject().(runtime.Object) //nolint
		_ = meta.NewAccessor().SetResourceVersion(original, "")

		modified = modified.DeepCopyObject().(runtime.Object) //nolint
		_ = meta.NewAccessor().SetResourceVersion(modified, version)
	}
